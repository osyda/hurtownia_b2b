  ...
  return (
    <div className="flex gap-6">
      ...
      <aside className="hidden md:block w-52 flex-shrink-0">
        <div className="bg-white rounded-xl border p-3 sticky top-24">
          ...
          {categories.map(cat => (
            <button
              ...
              className={cn(
                'w-full text-left px-3 py-2 rounded-lg text-sm transition-colors',
                activeCategory === cat.id ? 'font-semibold text-white' : 'text-gray-600 hover:bg-gray-50'
              )}
        ...
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
            />
          </div>
          <button type="submit" className="px-4 py-2.5 rounded-xl text-white text-sm font-medium hover:opacity-90 transition-opacity" style={{ backgroundColor: brandColor }}>
            Szukaj
          </button>
        </form>
        ...
        {products.length ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {products.map(product => {
              ...
              return (
                <div key={product.id} className={cn('bg-white rounded-xl border p-4 flex flex-col gap-3 transition-shadow hover:shadow-sm', unavailable && 'opacity-60')}>
