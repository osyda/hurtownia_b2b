...
export default async function NewCustomerPage({ params }: { params: Promise<{ tenant: string }> }) {
  ...
  const { data: priceGroups } = await supabase.from('price_groups').select('*').eq('tenant_id', profile.tenant_id).order('name')
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <CustomerForm tenantSlug={tenantSlug} priceGroups={priceGroups ?? []} onSubmit={submitAction} />
    </div>
  )
}
